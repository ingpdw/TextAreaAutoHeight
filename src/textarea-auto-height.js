...
exports = module.exports = class TextAreaAutoHeight {
	constructor( $node, $parent, parentRange = 0, lineHeight = 1.5 ) {
		this.$node = $node;
		this.$parent = $parent;
	...
	clone() {
		...
		this.$clone.removeAttr( 'id' ).removeAttr( 'name' )
			.css( { position:'absolute', top:0, left:-99999 } )
		...
		this.$node.on( 'focus', ( evt ) => {
			this.timer = setInterval( () => {
				if( this.$node.val() ){
					...
					nodeScrollH = ( nodeH <= nodeScrollH )?
						nodeScrollH + fontH: nodeH;
